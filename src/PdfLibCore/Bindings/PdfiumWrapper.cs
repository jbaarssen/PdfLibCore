...
namespace PdfLibCore.Bindings
{
    ...
    {
        public IntPtr __Instance;

        public FpdfDocumentT(IntPtr native)
        {
            __Instance = native;
        }

        public bool IsNull => __Instance == IntPtr.Zero;
        public FpdfDocumentT SetToNull() => new FpdfDocumentT(Interlocked.Exchange(ref __Instance, IntPtr.Zero));
    }
}
...
internal unsafe partial class fpdf_view
{
    public partial struct __Internal
    {
        ...
        internal static extern FpdfDocumentT FPDF_LoadMemDocument(ref byte data_buf, int size,
            [MarshalAs(UnmanagedType.LPStr)] string password);

        [SuppressUnmanagedCodeSecurity]
        [DllImport("pdfium", CallingConvention = CallingConvention.Cdecl,
            EntryPoint = "FPDF_LoadCustomDocument")]
        internal static extern FpdfDocumentT FPDF_LoadCustomDocument(FPDF_FILEACCESS pFileAccess,
            [MarshalAs(UnmanagedType.LPStr)] string password);
    ...
    public static FpdfDocumentT FPDF_LoadDocument(byte[] data, int index = 0, int count = -1, [MarshalAs(UnmanagedType.LPStr)] string password = null)
    {
        if (count < 0)
    ...
    public static FpdfDocumentT FPDF_LoadDocument(Stream stream, FPDF_FILEACCESS fileRead, int count = 0, [MarshalAs(UnmanagedType.LPStr)] string password = null)
    {
        return FPDF_LoadCustomDocument(fileRead, password);
    }

    public static FpdfDocumentT FPDF_LoadMemDocument(ref byte data_buf, int size, [MarshalAs(UnmanagedType.LPStr)] string password)
    {
        return __Internal.FPDF_LoadMemDocument(ref data_buf, size, password);
    }

    public static FpdfDocumentT FPDF_LoadCustomDocument(
        FPDF_FILEACCESS pFileAccess, [MarshalAs(UnmanagedType.LPStr)] string password)
    {
        return __Internal.FPDF_LoadCustomDocument(pFileAccess, password);
    }
...
internal unsafe partial class fpdf_ppo
{
        ...
        uint num_pages_on_x_axis, uint num_pages_on_y_axis)
    {
        ...
        return new FpdfDocumentT(__ret);
    }
